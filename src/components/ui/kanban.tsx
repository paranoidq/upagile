  ...
  return (
    <KanbanContext.Provider value={contextValue as KanbanContextValue<unknown>}>
      <DndContext
        ...
        onDragStart={composeEventHandlers(kanbanProps.onDragStart, ({ active }) => {
...
const KanbanColumn = React.forwardRef<HTMLDivElement, KanbanColumnProps>((props, forwardedRef) => {
  const { id = React.useId(), value, asChild, asHandle, disabled, className, style, ...columnProps } = props
...
const KanbanItem = React.forwardRef<HTMLDivElement, KanbanItemProps>((props, forwardedRef) => {
  const { id = React.useId(), value, style, asHandle, asChild, disabled, className, ...itemProps } = props
