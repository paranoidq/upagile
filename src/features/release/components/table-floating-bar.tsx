...
import { useDeleteReleases, useUpdateReleases } from '../_lib/services'
import { Release, releaseStatus } from '../types'

interface ReleaseTableFloatingBarProps {
  table: Table<Release>
}

export function ReleaseTableFloatingBar({ table }: ReleaseTableFloatingBarProps) {
  ...
  const { mutateAsync: updateReleases, isPending: isUpdatingReleases } = useUpdateReleases()

  const { mutateAsync: deleteReleases, isPending: isDeletingReleases } = useDeleteReleases()
            ...
            <Select
              onValueChange={(value: Release['status']) => {
                ...
                toast.promise(
                  ...
                  {
                    loading: 'Updating releases...',
                    success: 'Releases updated',
                    error: (e) => {
                      return {
                        message: e.msg,
                ...
                <SelectTrigger asChild>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      disabled={isUpdatingReleases}
                    >
                      {isUpdatingReleases && action === 'update-status' ? (
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  disabled={isDeletingReleases}
                >
                  {isDeletingReleases && action === 'delete' ? (
                    <Loader className='size-3.5 animate-spin' aria-hidden='true' />
                  ) : (
